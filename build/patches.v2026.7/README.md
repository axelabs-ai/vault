...
> **2026-09-05 (r5)**: 베이스를 `v2026.8.0-1` (`1bed3a1868bc`) 로 올렸다 — vaultwarden 1.37.2 상당(`fa2566d1` 까지 흡수, #7608 dummy revisionDate). Bitwarden 클라이언트 2026.7.0+/2026.8.0+ 가 1.37.0/1.37.2 미만 서버에서 **로그인 후 빈 금고**를 보이는 파손(#7615)의 수정. 0003/0004 는 그 태그에 `git apply --check` clean, sqlite 마이그레이션 추가 0건(롤백 = r4). 디렉터리명은 시리즈 번호 보존을 위해 유지. **원칙: Bitwarden 은 매달 클라이언트를 밀고 vaultwarden 은 그때마다 'required' 릴리즈를 낸다 — 이 베이스는 월 1회 최신 태그로 따라간다.**
